...
namespace _1_Calculator
{
    ...
    {
        private readonly Collection<string> computationHistory;
        ...
        private void Window_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.Key)
            {
                ...
                case Key.Delete:
                    ...
                    break;
                case Key.Oem2:
                    if (InputLanguageManager.Current.CurrentInputLanguage.ThreeLetterISOLanguageName == "eng" &&
                        !Keyboard.IsKeyDown(Key.LeftShift))
                        Calculator.ExecuteOperation(Calculator.CalculatorOperationType.Dividing);
                    else if (InputLanguageManager.Current.CurrentInputLanguage.ThreeLetterISOLanguageName == "rus" &&
                             !Keyboard.IsKeyDown(Key.LeftShift)) Calculator.EnterDot();
                    break;
                case Key.Oem5:
                    if (InputLanguageManager.Current.CurrentInputLanguage.ThreeLetterISOLanguageName == "rus" &&
                        Keyboard.IsKeyDown(Key.LeftShift))
                        Calculator.ExecuteOperation(Calculator.CalculatorOperationType.Dividing);
                    break;
                case Key.OemPeriod:
        ...
        {
            string logFilePath = Directory.GetCurrentDirectory() + @"\" + "Session_" +
                                 DateTime.Now.ToString("dd.MM.yyyy") + 'T' + DateTime.Now.ToString("hh.mm.ss") + ".txt";
            using (var streamWriter = new StreamWriter(logFilePath))
            {
                var i = 0;
                foreach (var computation in computationHistory)
                {
                    i++;
